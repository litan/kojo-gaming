...
class Platformer2 extends GdxGame {
  override def create(): Unit = {
    super.create()
    setScreen(new PlatformerScreen2())
  }
}

class PlatformerScreen2 extends PicGdxScreen {
  cleari()

  def scaleDown(p: BatchPics) = {
    p.scale(0.8)
    p
  }
...
//      pic.scale(0.8)
    ...
    val stillRight = scaleDown(picBatch(playerPicture(sheet.imageAt(0, 0))))
    val stillLeft = scaleDown(picBatch(playerPicture(sheet.imageAt(0, 1))))

    val runningRight = scaleDown(
      picBatch(
        List(
          sheet.imageAt(0, 2),
          sheet.imageAt(1, 2),
