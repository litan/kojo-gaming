...
package object tiles {
  ...
  case class Level(world: TileWorld) {
  ...
  case class Layer(index: Int, world: TileWorld) {
  ...
  // Kojo picture tile
  case class Tile(kx: Double, ky: Double, tx: Int, ty: Int, image: BufferedImage) {
    val tilePic = Picture.image(image)
  ...
  class TileWorld(fileName: String) {
...
//      val p = Utils.absolutePath(fileName)
    ...
    def tileAtLeft(pic: Picture, layerIdx: Int) = {
      val b = pic.boundaryPolygon.getBoundingRectangle
